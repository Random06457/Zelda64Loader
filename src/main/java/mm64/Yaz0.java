		...
		try
		{
			...
			while (out.position() < outSize)
			{
                ...
                else
                {
                    ...
                    //Log.info( String.format("raw=0x%X; nibble=0x%X; backOff=0x%X; backSize=0x%X; tmpPos=0x%X, newPos=0x%X", raw, nibble, backOff, backSize, tmpPos, newPos));
